...
public class ListViewOrderComment extends LinearLayout {
    List<OrderModel_goods_data> data = new ArrayList<>();
    String targetPic;
    ListViewOrderCommentCallBack listViewOrderCommentCallBack;
    LoadImageCallBack loadImageCallBack;

    public ListViewOrderComment(Context context) {
        super(context);
    }

    public ListViewOrderComment(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public ListViewOrderComment(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public List<OrderModel_goods_data> getData() {
        return data;
    }

    public void setList(Context context, List<OrderModel_goods_data> data, String targetPic) {
        this.removeAllViews();
        this.data = data;
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            ratingBar.setOnStarChangeListener(new OnChangeListener() {
                ...
                public void onChange(int star) {
                    switch (star) {
                        case 1:
                            data.get(finalI).customGrade= String.valueOf(star);
                        ...
                        case 5:
                            ...
                            break;
                        default:
                            data.get(finalI).customGrade=null;
            ...
            input_item_listview_order_comment_adapter.addTextChangedListener(new TextWatcher() {
                ...
                public void afterTextChanged(Editable s) {
                    data.get(finalI).customContent = input_item_listview_order_comment_adapter.getText().toString().trim();
                }
            });
            ...
            HorizontalListView_item_listview_order_comment_adapter.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    if ("default".equals(data.get(finalI).photo.get(position))){
                        LogUtils.e("onAddPhoto");
                        if (listViewOrderCommentCallBack!=null){
                            listViewOrderCommentCallBack.onAddPhoto(finalI,position,data);
                        }
                    }else {
                        LogUtils.e("onClickImage");
                        if (listViewOrderCommentCallBack!=null){
                            listViewOrderCommentCallBack.onClickImage(finalI,position,data);
                        }
                    }
                }
            });
            sss_adapter.setList(data.get(finalI).photo);
    ...
    public interface ListViewOrderCommentCallBack{
        void onClickImage(int finaI, int position, List<OrderModel_goods_data> data);

        void onAddPhoto(int finaI, int position, List<OrderModel_goods_data> data);
    }

...
public class FragmentOrderSOSBuyer extends BaseFragment implements RefreshLoadMoreLayout.CallBack {
    public static final int FragmentOrderSOSBuyer_Service_unFinish = 1;//1服务中，
    public static final int FragmentOrderSOSBuyer_Service_complete = 2;//2服务已完成
    ...
    public FragmentOrderSOSBuyer(int status) {
        this.status = status;
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        scollViewFragmentOrderSOSBuyer = null;
        refreshFragmentOrderSOSBuyer = null;
        topFragmentOrderSOSBuyer = null;
        listviewFragmentOrderSOSBuyer = null;
    ...
    public void changeList(String sos_id) {
        ...
        if (list.size() > 0) {
            ...
            listviewFragmentOrderSOSBuyer.removeAllViewsInLayout();
        }

    }

    void init() {
        refreshFragmentOrderSOSBuyer.init(new RefreshLoadMoreLayout.Config(this).canLoadMore(true).canRefresh(true));
        scollViewFragmentOrderSOSBuyer.setSmoothScrollingEnabled(true);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            scollViewFragmentOrderSOSBuyer.setOnScrollChangeListener(new View.OnScrollChangeListener() {
                @Override
                public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                    if (scrollY > Config.scoll_HighRestriction) {
                        topFragmentOrderSOSBuyer.setVisibility(View.VISIBLE);
                    } else {
                        topFragmentOrderSOSBuyer.setVisibility(View.GONE);
                    }
                }
            });
        }
        topFragmentOrderSOSBuyer.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                scollViewFragmentOrderSOSBuyer.smoothScrollTo(0, 0);
            }
        });
    }

    void initAdapter() {
        if (status == FragmentOrderSOSBuyer_Service_unFinish) {
            sss_adapter = new SSS_Adapter<SOSSellerOrderModel>(getBaseFragmentActivityContext(), R.layout.item_fragment_order_sos_seller_unfinish_adapter, list) {
                ...
                protected void setView(SSS_HolderHelper helper, int position, SOSSellerOrderModel bean, SSS_Adapter instance) {
                    ...
                    helper.getView(R.id.nav_fragment_order_sos_seller_adapter).setVisibility(View.GONE);
                    addImageViewList(FrescoUtils.showImage(false, 100, 100, Uri.parse(Config.url + bean.face), (SimpleDraweeView) helper.getView(R.id.pic_item_fragment_order_sos_seller_adapter), 0f));
                    addImageViewList(FrescoUtils.showImage(false, 100, 100, Uri.parse("res://" + AppUtils.getAppPackageName(getBaseFragmentActivityContext()) + "/" + R.mipmap.logo_qr),
...
//                            }
        ...
        listviewFragmentOrderSOSBuyer.setAdapter(sss_adapter);
    }
            ...
            addRequestCall(new RequestModel(System.currentTimeMillis() + "", RequestWeb.getSOSSellerList(
                    ...
                    , new StringCallback() {
                        ...
                        public void onError(Call call, Exception e, int id) {
                            ...
                            if (refreshFragmentOrderSOSBuyer != null) {
                                refreshFragmentOrderSOSBuyer.stopRefresh();
                                refreshFragmentOrderSOSBuyer.stopLoadMore();
                            }
                            ToastUtils.showShortToast(getBaseFragmentActivityContext(), e.getMessage());
                        }

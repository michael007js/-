...
public class FragmentSaleAndSeckillFilterNow extends BaseFragment implements RefreshLoadMoreLayout.CallBack{
    @BindView(R.id.listview_linearlayout_fragment_sale_and_seckill_filter_now)
    InnerListview listviewFragmentSaleAndSeckillFilterNow;
    @BindView(R.id.scoll_view_fragment_sale_and_seckill_filter_now)
    ScrollView scollViewFragmentSaleAndSeckillFilterNow;
    @BindView(R.id.refresh_fragment_sale_and_seckill_filter_now)
    RefreshLoadMoreLayout refreshFragmentSaleAndSeckillFilterNow;
    @BindView(R.id.top_fragment_sale_and_seckill_filter_now)
    ImageView topFragmentSaleAndSeckillFilterNow;
    Unbinder unbinder;
SSS_Adapter sss_adapter;
    ...
    public FragmentSaleAndSeckillFilterNow(String classify_id, String type) {
        this.classify_id = classify_id;
        this.type=type;
    }

    public FragmentSaleAndSeckillFilterNow() {
    }
    ...
    public void onDestroy() {
        ...
        sss_adapter=null;
        listviewFragmentSaleAndSeckillFilterNow = null;
        scollViewFragmentSaleAndSeckillFilterNow = null;
        refreshFragmentSaleAndSeckillFilterNow = null;
        topFragmentSaleAndSeckillFilterNow = null;
    ...
    protected int setContentView() {
        return R.layout.fragment_sale_and_seckill_filter_now;
    }
                ...
                public void run() {
                    ...
                    try {
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                refreshFragmentSaleAndSeckillFilterNow.init(new RefreshLoadMoreLayout.Config(FragmentSaleAndSeckillFilterNow.this).canRefresh(true).canLoadMore(true));
                                initAdapter();
                                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                                    scollViewFragmentSaleAndSeckillFilterNow.setOnScrollChangeListener(new View.OnScrollChangeListener() {
                                        @Override
                                        public void onScrollChange(View v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                                            if (scrollY > Config.scoll_HighRestriction) {
                                                topFragmentSaleAndSeckillFilterNow.setVisibility(View.VISIBLE);
                                            } else {
                                                topFragmentSaleAndSeckillFilterNow.setVisibility(View.GONE);
                                            }
                                        }
                                    });
                                    topFragmentSaleAndSeckillFilterNow.setOnClickListener(new View.OnClickListener() {
                                        @Override
                                        public void onClick(View v) {
                                            scollViewFragmentSaleAndSeckillFilterNow.smoothScrollTo(0, 0);
                                        }
                                    });
                                }
    ...
    void initAdapter(){
        ...
        listviewFragmentSaleAndSeckillFilterNow.setAdapter(sss_adapter);
        ...
        try {
            addRequestCall(new RequestModel(System.currentTimeMillis() + "", RequestWeb.activity_list(
                    new JSONObject()
                            ...
                            .put("activity", "2")/*（热门榜单activity= 1）（进行中activity= 2）（即将开场activity= 3）*/
                        ...
                        public void onError(Call call, Exception e, int id) {
                            ...
                            if (refreshFragmentSaleAndSeckillFilterNow!=null){
                                refreshFragmentSaleAndSeckillFilterNow.stopLoadMore();
                                refreshFragmentSaleAndSeckillFilterNow.stopRefresh();
                            }

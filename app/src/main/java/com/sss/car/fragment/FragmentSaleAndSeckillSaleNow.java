...
public class FragmentSaleAndSeckillSaleNow extends BaseFragment {
    @BindView(R.id.listViewSale_fragment_sale_and_seckill_sale_now)
    LinearLayout listViewSaleFragmentSaleAndSeckillSaleNow;
    Unbinder unbinder;
    ...
    String activity = "2";/*（热门榜单activity= 1）（进行中activity= 2）（即将开场activity= 3）*/


    public FragmentSaleAndSeckillSaleNow(String classify_id, String type) {
        this.classify_id = classify_id;
        this.type = type;
    }

    public FragmentSaleAndSeckillSaleNow() {
    }

    @Override
    protected int setContentView() {
        return R.layout.fragment_sale_and_seckill_sale_now;
    }
    ...
    public void onDestroy() {
        ...
        listViewSaleFragmentSaleAndSeckillSaleNow = null;
        classify_id = null;
        super.onDestroy();
    }
    ...
    void showGoodsList(final List<ListViewShowGoodsServiceModel> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
                        ...
                        case R.id.get_item_sale_and_seckill_adapter:
                            if (getBaseFragmentActivityContext() != null) {
                                startActivity(new Intent(getBaseFragmentActivityContext(), ActivityGoodsServiceDetails.class)
                                        .putExtra("type", type)
                                        .putExtra("goods_id", list.get(finalI1).list.get(position).goods_id)
                                );
                            }
                            break;
                    }
            ...
            listViewSaleFragmentSaleAndSeckillSaleNow.addView(listViewSale);

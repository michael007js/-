...
public class OrderSOSPublish extends BaseActivity {
    ...
    protected void onDestroy() {
        ...
        rightButtonTop = null;
        isExistOrderSosPublish = null;
        clickOneKeyWriteCarOrderSosPublish = null;
        clickOneKeyLocationOrderSosPublish = null;
        clickOneKeyWriteAddressOrderSosPublish = null;
        peopleNameOrderSosPublish = null;
        mobileNameOrderSosPublish = null;
        carOrderSosPublish = null;
        clickChooseCarOrderSosPublish = null;
        showTypeOrderSosPublish = null;
        clickTypeOrderSosPublish = null;
        showAddressOrderSosPublish = null;
        clickAddressOrderSosPublish = null;
        priceOrderSosPublish = null;
        showTimeOrderSosPublish = null;
        clickTimeOrderSosPublish = null;
        showPenalSumOrderSosPublish = null;
        clickPenalSumOrderSosPublish = null;
        showOtherOrderSosPublish = null;
        clickOtherSumOrderSosPublish = null;
        photo = null;
        tipOrderSosPublish = null;
        totalPriceOrderSosPublish = null;
        clickSubmitOrderSosPublish = null;
        orderSosPublish = null;
        super.onDestroy();
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        customInit(orderSosPublish, false, true, true);
        ...
        APPOftenUtils.underLineOfTextView(clickOneKeyWriteCarOrderSosPublish).setText("手工填写");
        APPOftenUtils.underLineOfTextView(clickOneKeyLocationOrderSosPublish).setText("一键定位");
        APPOftenUtils.underLineOfTextView(clickOneKeyWriteAddressOrderSosPublish).setText("手工填写");
        menuDialog = new MenuDialog(this);
        priceOrderSosPublish.init(getBaseActivityContext(), true)
                .minNumber(0)
                ...
                .setNumberSelectEditOperationCakkBack(new NumberSelectEdit.NumberSelectEditOperationCakkBack() {
                    ...
                    public void onAdd(NumberSelectEdit numberSelectEdit, int currentNumber) {
                        totalPriceOrderSosPublish.setText(currentNumber + ".00");
                    }
                    ...
                    public void onZero(NumberSelectEdit numberSelectEdit) {
                        totalPriceOrderSosPublish.setText("0.00");
                    }
...
//        carOrderSosPublish.setText(event.carName);
        defaultVehicle();
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.right_button_top://保存
                saveSOS();
                break;
            case R.id.click_choose_car_order_sos_publish://选择车辆
                if (getBaseActivityContext() != null) {
                    startActivity(new Intent(getBaseActivityContext(), ActivityMyDataCarGarage.class));
                }
                break;
            case R.id.click_type_order_sos_publish://服务类型
                serviceType();
                break;
            case R.id.click_address_order_sos_publish://点击地址
                if (getBaseActivityContext() != null) {
                    startActivity(new Intent(getBaseActivityContext(), ActivityChooseAdress.class));
                }
                break;
            case R.id.click_time_order_sos_publish://服务就位时间
                serviceTime();
                break;
            case R.id.click_penal_sum_order_sos_publish://违约金
                integrityMoney();
                break;
            case R.id.click_other_sum_order_sos_publish://其他要求
                if (getBaseActivityContext() != null) {
                ...
                break;
            case R.id.click_submit_order_sos_publish://提交订单
                publicSOS("1");//0草稿箱，1立即求助（求助中），2进行中，3已完成
                break;
            case R.id.click_one_key_write_car_order_sos_publish://手工填写车辆信息
                if (getBaseActivityContext() != null) {
                    startActivity(new Intent(getBaseActivityContext(), ActivityMyDataCarGarage.class));
                }
                break;
            case R.id.click_one_key_location_order_sos_publish://一键定位
                encodingToString();
                break;
...
//                        LogUtils.e(targetInfoModel.toString());
                        showAddressOrderSosPublish.setText(poiResult.getPois().get(j).getCityName() + poiResult.getPois().get(j).getTitle());
                    }
                } else {
                    ...
                    case R.id.click_item_dialog_coupons:
                        for (int i = 0; i < integrityMoneyList.size(); i++) {
                            ...
                            if (i == position) {
                                ...
                                showPenalSumOrderSosPublish.setText(integrityMoneyList.get(i).name);
                            } else {
                    ...
                    case R.id.click_item_dialog_coupons:
                        for (int i = 0; i < serviceTimeList.size(); i++) {
                            ...
                            if (i == position) {
                                ...
                                showTimeOrderSosPublish.setText(serviceTimeList.get(i).name);
                            } else {
                    ...
                    case R.id.click_item_dialog_coupons:
                        for (int i = 0; i < serviceTypeList.size(); i++) {
                            ...
                            if (i == position) {
                                ...
                                showTypeOrderSosPublish.setText(serviceTypeList.get(i).name);
                            } else {
...
//        photo.setAdapter(sss_adapter);
                        ...
                        public void onResponse(String response, int id) {
                            ...
                            try {
                                ...
                                if ("1".equals(jsonObject.getString("status"))) {
                                    peopleNameOrderSosPublish.setText(jsonObject.getJSONObject("data").getString("username"));
                                    mobileNameOrderSosPublish.setText(jsonObject.getJSONObject("data").getString("mobile"));
                                    carOrderSosPublish.setText(jsonObject.getJSONObject("data").getString("vehicle_name"));
                                } else {
                                    peopleNameOrderSosPublish.setText("");
                                    mobileNameOrderSosPublish.setText("");
                                    carOrderSosPublish.setText("");
                                    isExistOrderSosPublish.setVisibility(View.GONE);
                                    noExistOrderSosPublish.setVisibility(View.VISIBLE);
                                }

                                if (!StringUtils.isEmpty(carOrderSosPublish.getText().toString().trim())) {
                                    isExistOrderSosPublish.setVisibility(View.VISIBLE);
                                    noExistOrderSosPublish.setVisibility(View.GONE);
                                } else {
                                    isExistOrderSosPublish.setVisibility(View.GONE);
                                    noExistOrderSosPublish.setVisibility(View.VISIBLE);
                                }
                    ...
                    new JSONObject()
                        ...
                        public void onResponse(String response, int id) {
                            ...
                            try {
                                JSONObject jsonObject = new JSONObject(response);
                                if ("1".equals(jsonObject.getString("status"))) {
                                    tipOrderSosPublish.setText(jsonObject.getJSONObject("data").getString("contents"));
                                } else {
        ...
        if (jsonArray.length() > 0) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if ("1".equals(integrityMoneyModel.is_check)) {
                    showPenalSumOrderSosPublish.setText(integrityMoneyModel.name);
                }
    ...
    void parseToServiceTimeList(JSONObject jsonObject) throws JSONException {
        ...
        if (jsonArray.length() > 0) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                integrityMoneyModel.name = jsonArray.getJSONObject(i).getString("name");
                if ("1".equals(integrityMoneyModel.is_check)) {
                    showTimeOrderSosPublish.setText(integrityMoneyModel.name);
                }
    ...
    void parseToServiceTypeList(JSONObject jsonObject) throws JSONException {
        ...
        if (jsonArray.length() > 0) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                integrityMoneyModel.name = jsonArray.getJSONObject(i).getString("name");
                if ("1".equals(integrityMoneyModel.is_check)) {
                    showTypeOrderSosPublish.setText(integrityMoneyModel.name);
                }
    ...
    public void publicSOS(String status) {
        ...
        if (StringUtils.isEmpty(showTypeOrderSosPublish.getText().toString().trim())) {
        ...
        if (priceOrderSosPublish.getCurrentNumber() < 1) {
            ToastUtils.showShortToast(getBaseActivityContext(), " 请选择您的服务价格");
            return;
        }

        if (StringUtils.isEmpty(sendLai) || StringUtils.isEmpty(sengLng) || StringUtils.isEmpty(showAddressOrderSosPublish.getText().toString().trim())) {
            ToastUtils.showShortToast(getBaseActivityContext(), " 请选择您的地址");
            return;
        }
        if (StringUtils.isEmpty(showTimeOrderSosPublish.getText().toString().trim())) {
            ToastUtils.showShortToast(getBaseActivityContext(), " 请选择服务就位时间");
            return;
        }
        if (StringUtils.isEmpty(showPenalSumOrderSosPublish.getText().toString().trim())) {
        ...
        try {
            addRequestCall(new RequestModel(System.currentTimeMillis() + "", RequestWeb.publisSOS(
                    new JSONObject()
                            ...
                            .put("mobile", mobileNameOrderSosPublish.getText().toString().trim())

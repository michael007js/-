...
public class ActivityMyDataCarYear extends BaseActivity implements MyCarDisplacementYearTypeClickCallBack {
    ...
    @BindView(R.id.logo_activity_my_data_car_car_year)
    ImageView logoActivityMyDataCarCarYear;
    @BindView(R.id.name_activity_my_data_car_car_year)
    TextView nameActivityMyDataCarCarYear;
    @BindView(R.id.list_activity_my_data_car_car_year)
    ListView listActivityMyDataCarCarYear;
    @BindView(R.id.activity_my_data_car_car_year)
    LinearLayout activityMyDataCarCarYear;
    ...
    protected void onDestroy() {
        ...
        titleTop = null;
        rightButtonTop = null;
        logoActivityMyDataCarCarYear = null;
        nameActivityMyDataCarCarYear = null;
        listActivityMyDataCarCarYear = null;
        activityMyDataCarCarYear = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_my_data_car_car_year);
        ButterKnife.bind(this);
        customInit(activityMyDataCarCarYear, false, true, true);
        titleTop.setText("年份选择");
        if (getIntent() == null || getIntent().getExtras() == null || getBaseActivityContext() == null) {
        ...
        listActivityMyDataCarCarYear.setAdapter(carDisplacementYearTypePublicAdapter);
        logoActivityMyDataCarCarYear.setTag(R.id.glide_tag, Config.url + getIntent().getExtras().getString("carLogo"));
        if (getBaseActivityContext() != null) {
            addImageViewList(GlidUtils.downLoader(false, logoActivityMyDataCarCarYear, getBaseActivityContext()));
        }
        nameActivityMyDataCarCarYear.setText(getIntent().getExtras().getString("carType") + getIntent().getExtras().getString("carLabel"));
/*
  .putExtra("id", id)
                .putExtra("carType",getIntent().getExtras().getString("carType"))
                .putExtra("carLabel",getIntent().getExtras().getString("name"))
                .putExtra("carDisplacement", name)
                .putExtra("carLogo", logo));
* */
        getYear(getIntent().getExtras().getString("id"), getIntent().getExtras().getString("carLogo"));
    }
     ...
     * @param id
     */
    void getYear(String id, final String logo) {
        if (ywLoadingDialog != null) {
            ywLoadingDialog.disMiss();
        }
        ywLoadingDialog = null;
        ...
        try {
            addRequestCall(new RequestModel(System.currentTimeMillis() + "", RequestWeb.chooseCarYear(
                    new JSONObject()
                            .put("property_id", id)
    ...
    public void onClickCar(String id, String name, String logo) {
        startActivity(new Intent(getBaseActivityContext(), ActivityMyDataCarType.class)
                .putExtra("id", id)
                .putExtra("ids", getIntent().getExtras().getString("ids")+","+id)
                .putExtra("carType",getIntent().getExtras().getString("carType"))
                .putExtra("carLabel",getIntent().getExtras().getString("carLabel"))
                .putExtra("carDisplacement", getIntent().getExtras().getString("carDisplacement"))
                .putExtra("carYear",name)
                .putExtra("carLogo", logo));
    }

...
public abstract class SSS_Adapter<T> extends BaseAdapter {
    ...
    public SSS_Adapter(Context context, int itemLayoutId) {
        this(context);
        mItemLayoutId = itemLayoutId;

    }

    public SSS_Adapter(Context context, int itemLayoutId, List<T> mList) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        SSS_HolderHelper helper = SSS_HolderHelper.holderHelperByConvertView(view, viewGroup, mItemLayoutId);


        helper.setOnItemListener(mOnItemListener);
        helper.setPosition(i);
        setItemListener(helper);
        setView(helper, i, getItem(i),this);
        if (sss_adapterGetViewCallBack!=null){
            sss_adapterGetViewCallBack. onGetView(this,helper, i, getItem(i));
        }
    ...
    protected abstract void setView(SSS_HolderHelper helper, int position, T bean,SSS_Adapter instance);

    protected abstract void setItemListener(SSS_HolderHelper helper);

    public interface SSS_AdapterGetViewCallBack{
        void onGetView(SSS_Adapter sss_adapter, SSS_HolderHelper helper, int position, Object bean);
    }
}

...
public class AutoUtils {
    ...
    /**
     * @param act 要适配的activity,需在 setContentView之前调用
     * @param hasStatusBar 是否有状态栏
     */
    public static void setSize(Activity act, boolean hasStatusBar) {
        int designWidth=act.getWindowManager().getDefaultDisplay().getWidth();
        int designHeight=act.getWindowManager().getDefaultDisplay().getHeight();
    ...
    /**
     * @param act 要适配的activity
     * @param hasStatusBar 是否有状态栏
     * @param desisgnWidth 设计宽度
     * @param desisgnHeight 设计高度
     */
    public static void setSize(Activity act, boolean hasStatusBar, int desisgnWidth, int desisgnHeight) {
        if (act == null || designWidth < 1 || designHeight < 1) return;
    ...
    public static int getStatusBarHeight(Context context) {
     ...
     static int getDisplayWidthValue(int designWidthValue) {
     ...
     static int getDisplayHeightValue(int designHeightValue) {

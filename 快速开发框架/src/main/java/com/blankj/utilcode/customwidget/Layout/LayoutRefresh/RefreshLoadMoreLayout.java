package com.blankj.utilcode.customwidget.Layout.LayoutRefresh;
...
import com.blankj.utilcode.customwidget.Layout.LayoutRefresh.rlm.RlmScrollView;
...
public class RefreshLoadMoreLayout extends ViewGroup {
    ...
    private boolean mCanRefresh;//是否允许下拉刷新

    private FooterLayout mFooterLayout;
    private boolean mCanLoadMore;//是否允许加载更多

    private boolean mIncludeScrollHorizontal;//是否兼容横向滑动

    private boolean mIncludeScrollY;//如果Y轴方向发生滑动冲突,则可以开启此开关
    ...
    private void initCommonViews(TypedArray typedArray) {
        int[] colors = new int[]{
                getColor(R.color.rll_layout_bg_color), getColor(R.color.black)
        };
    ...
    private void initHeaderViews(TypedArray typedArray) {
        ...
        Drawable[] drawables = new Drawable[]{
                getDrawable(R.drawable.arrow_new), getDrawable(R.drawable.rll_progress)
        };
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mFirstMove && (mJudgeYDistance = Math.abs(mNowYPos - mDownYPos)) < mTouchSlop) {
                    LogUtils.e("error distance %f, should more than %d",
                            mJudgeYDistance,
                            mTouchSlop);
                    mFirstMove = false;
                    return false;
                ...
                if (isPullDown(MotionEvent.ACTION_MOVE, mYDistance)) {
                    LogUtils.e("pull down, intercept touch event");
                    return true;
                } else if (isPullUp(MotionEvent.ACTION_MOVE, mYDistance)) {
                    LogUtils.e("pull up, intercept touch event");
                    return true;
                }
...
/*******************************这里修复的嵌套到InCludeLandscapeScrollView中无法上下滑动的问题*************************************/
//                if (mIncludeScrollY) {
////                    LogUtils.e(scrollOldX+"---"+scrollX+"---"+scrollOldY+"---"+scrollY);
//
//                    //判断有水平滑动的意向
//                    int new_x = (int) ev.getX();
//                    int new_y = (int) ev.getY();
//                    int move_x = xT - new_x;//x轴滑动的距离
//                    int move_y = new_y - yT;//y轴滑动的距离
//                    if (move_x > 50 && move_y < 4) {//50的偏移量
//                        getParent().requestDisallowInterceptTouchEvent(false);
//                    } else if (move_x < -50 && move_y < 4) {//50的偏移量
//                        getParent().requestDisallowInterceptTouchEvent(false);
//                    } else {
//                        //判断有垂直滑动的意向且达到一定的距离
//                        if (move_x < 50 &&move_y > -8 || move_x < 50 && move_y<8) {
...
/*******************************这里修复的嵌套到InCludeLandscapeScrollView中无法上下滑动的问题*************************************/
    ...
    public void init(Config config) {
        ...
        setLoadMoreLayout();
        includeScrollHorizontal(config.mIncludeScrollHorizontal);
        includeScrollY(config.mIncludeScrollY);
    ...
    public static class Config {
        ...
        private boolean mShowLastRefreshTime = true;
        private String mKeyLastRefreshTime = "";
        private String mHeaderDateFormat = "yyyy-MM-dd HH:mm:ss";
        private boolean mCanLoadMore = true;
        private boolean mAutoLoadMore = false;
        private boolean mMultiTask = false;
        private boolean mIncludeScrollHorizontal = false;
        private boolean mIncludeScrollY = false;
         ...
         * 是否兼容横向滑动
         *
         * @param includeScrollHorizontal
         */
        public Config includeScrollHorizontal(boolean includeScrollHorizontal) {
            mIncludeScrollHorizontal = includeScrollHorizontal;
            return this;
        }

        /**
         * 如果遇到Y轴方向滑动冲突,则可以开启此开关
         *
         * @param includeScrollY
         */
        public Config includeScrollY(boolean includeScrollY) {
            this.mIncludeScrollY = includeScrollY;
            return this;
        }
    ...
    public void includeScrollHorizontal(boolean includeScrollHorizontal) {
        mIncludeScrollHorizontal = includeScrollHorizontal;
    }

    public void includeScrollY(boolean includeScrollY) {
        this.mIncludeScrollY = includeScrollY;
    }
        ...
        if (cv instanceof RecyclerView) {
            ((RecyclerView) cv).addOnScrollListener(new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    ...
                    LogUtils.e("RecyclerView,newState=%d", newState);
                    if (b) {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    ...
                    LogUtils.e("RecyclerView, dy=%d", dy);
                    if (dy > 0 && isCanLoadMore() && RefreshLoadMoreUtil.isContentToBottom(cv) || dy < 0 && isCanRefresh() && RefreshLoadMoreUtil
                        ...
                        recyclerView.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                LogUtils.e("RecyclerView:allow intercept touch event");
            ...
            ((AbsListView) cv).setOnScrollListener(new AbsListView.OnScrollListener() {
                ...
                public void onScroll(AbsListView view,
                                     ...
                                     int totalItemCount) {
                    if (isCanLoadMore() && RefreshLoadMoreUtil.isContentToBottom(cv) || isCanRefresh() && RefreshLoadMoreUtil.isContentToTop(cv)) {
                        LogUtils.e("AbListView:allow intercept touch event" + firstVisibleItem + visibleItemCount + totalItemCount);
                        requestDisallowInterceptTouchEvent(false);
                    }
                }
            });
                ...
                public void onScrollChanged(ScrollView scrollView,
                    ...
                    if (t - oldt > 0 && isCanLoadMore() && RefreshLoadMoreUtil.isContentToBottom(cv)) {//滚动条向下滚动
                        if (b) {
                            ...
                            LogUtils.e("RlmScrollView:allow intercept touch event");
                            requestDisallowInterceptTouchEvent(false);
                        }

 ...
 *******************************************************************************/
package com.blankj.utilcode.pullToRefresh.internal;
...
import com.blankj.utilcode.pullToRefresh.ILoadingLayout;
import com.blankj.utilcode.pullToRefresh.PullToRefreshBase.Mode;
import com.blankj.utilcode.pullToRefresh.PullToRefreshBase.Orientation;
...
public abstract class LoadingLayout extends FrameLayout implements
	...
	private SharedPreferences sharedPreferences = getContext()
			.getSharedPreferences("refreshtime", Context.MODE_PRIVATE);
	private Editor editor = sharedPreferences.edit();// 获取编辑器
	...
	public LoadingLayout(Context context, final Mode mode,
		...
		mTimeText = (TextView) findViewById(R.id.refresh_time_text);
	...
	public void showRefreshTime() {
		long lasttime = sharedPreferences.getLong("longtime", 0);
		long timemils = System.currentTimeMillis();
		if (lasttime == 0) {
			refreshtime = "";
		} else {
//			Log.v("TAG", lasttime + "");
//			long temp = timemils - lasttime;
//			if (temp > 0 && temp < 60 * 1000) {
//				refreshtime = "更新于" + temp / 1000 + "秒前";
//			} else if (temp < 2 * 60 * 60 * 1000) {
//				refreshtime = "更新于" + temp / (60 * 1000) + "分钟前";
//			} else if (temp < 6 * 60 * 60 * 1000) {
//				SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
//				refreshtime = "更新于" + sdf.format(lasttime);
//			} else {
//				SimpleDateFormat sdf1 = new SimpleDateFormat(
//						"yyyy-MM-dd HH:mm:ss");
//				refreshtime = "更新于" + sdf1.format(lasttime);
//			}
			refreshtime="上次更新时间:"+TimeUtils.millis2String(lasttime,"yyyy-MM-dd HH:mm:ss");
		}
		editor.clear();
		editor.putLong("longtime", timemils);
		editor.commit();
		SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
		mTimeText.setText(refreshtime);
	}

	public void setRefreshTime() {
		editor.clear();
		editor.putLong("longtime", System.currentTimeMillis());
		editor.commit();

	}

	public void setTimeVisible(boolean b) {
		mTimeText.setVisibility(b ? View.VISIBLE : View.GONE);
	}

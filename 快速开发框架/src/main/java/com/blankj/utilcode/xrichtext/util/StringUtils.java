package com.blankj.utilcode.xrichtext.util;
...
public class StringUtils {
    ...
    public static String getImage(String content) {
        return content.substring(content.indexOf("<img src='"), content.indexOf(">"));
    ...
    public static String getImgSrc(String content) {
        ...
        if (result_img) {
            while (result_img) {
                ...
                //开始匹配<img />标签中的src
                Pattern p_src = Pattern.compile("<img[^>]+src\\\\s*=\\\\s*['\\\"]([^'\\\"]+)['\\\"][^>]*>");//(src|SRC)=("|')(.*?)("|')
